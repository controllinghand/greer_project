...
def main():
    ...
    st.markdown(
        """
        Stock-only community page for **YRVG** (buys & sells of shares).  
        Uses **only**: portfolios, portfolio_events, portfolio_nav_daily.

        **Strategy:**  
        Portfolio holdings are systematically sourced from the same **Weekly IV Targets**
        process used by **YRVI**, but instead of selling options for income, this fund
        buys and holds shares for long-term growth. The goal is to test the same
        high-conviction weekly setups through a pure equity ownership approach.
        """
    )

    default_code = "YRVG-26"
    ...
    if not p0.empty and pd.notna(p0.iloc[0]["start_date"]):
        try:
            ...
            db_start = None
    default_start_date = db_start or date(2026, 3, 23)
    ...
    # ----------------------------------------------------------
    # Growth Rules Section (YRVG only)
    # ----------------------------------------------------------
    st.divider()
