...
st.set_page_config(page_title="YRSPY-26 Results", layout="wide")

# ----------------------------------------------------------
# Main
# ----------------------------------------------------------
def main():
    st.title("📊 YRSPY-26 Results (You Rock SPY 500 Benchmark Fund est. 2026)")

    st.markdown(
        """
        **YRSPY-26** is the You Rock Club benchmark fund.  
        Strategy: **Buy $100,000 of SPY at the start of 2026 and hold.**

        Stock-only community page (buys & sells of shares).  
        Uses **only**: portfolios, portfolio_events, portfolio_nav_daily.
        """
    )

    default_code = "YRSPY-26"
    ...
    # Benchmark fund start date should be Jan 1, 2026 (fallback)
    ...
    with st.sidebar:
        ...
        st.divider()
        st.caption("Benchmark rule reminder:")
        st.markdown("- Buy **QQQ** with **$100,000**\n- Hold (no active trading)")
    ...
    # Full NAV history from DB start (for year blocks + downloads)
    ...
    # Year summary for 2026 right after header
    st.divider()
