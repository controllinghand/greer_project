# build_sector_summary_daily.py
# ----------------------------------------------------------
# build_sector_summary_daily.py
# - Computes daily sector breadth + Greer Market Regime Score by sector
# - Source: dashboard_snapshot
# - Inserts into: sector_summary_daily (1 row per sector per day)
# ----------------------------------------------------------
...
# ----------------------------------------------------------
# Function to compute regime score
# ----------------------------------------------------------
...
# ----------------------------------------------------------
# Function to summarize one sector
# ----------------------------------------------------------
def summarize_sector(sector_df: pd.DataFrame, summary_date):
    total = int(len(sector_df))

    above50 = pd.to_numeric(sector_df["above_50_count"], errors="coerce").fillna(0)
    gv_score = pd.to_numeric(sector_df["greer_value_score"], errors="coerce").fillna(0)
    ys_score = pd.to_numeric(sector_df["greer_yield_score"], errors="coerce").fillna(0).astype(int)

    # ----------------------------------------------------------
    # Buckets: GV
    # ----------------------------------------------------------
    gv_bucket = pd.Series("red", index=sector_df.index)
    ...
    # Buckets: YS
    # ----------------------------------------------------------
    ys_bucket = pd.Series("red", index=sector_df.index)
    ...
    # Buckets: GFV
    # ----------------------------------------------------------
    gfv_norm = sector_df["gfv_status"].astype("string").fillna("gray").str.strip().str.lower()
    ...
    # BuyZone
    # ----------------------------------------------------------
    buyzone = sector_df["buyzone_flag"].astype("boolean").fillna(False)
    ...
    ys_gold_pct = safe_pct(ys_gold, total)
    ys_bullish_pct = ys_green_pct + ys_gold_pct

    gfv_green_pct = safe_pct(gfv_green, total)
    gfv_gold_pct = safe_pct(gfv_gold, total)
    gfv_bullish_pct = gfv_green_pct + gfv_gold_pct
    ...
    return {
        ...
        "gv_bullish_pct": gv_bullish_pct,
        "ys_bullish_pct": ys_bullish_pct,
        "gfv_bullish_pct": gfv_bullish_pct,
    }
...
def main():
    ...
    ddl = """
    CREATE TABLE IF NOT EXISTS sector_summary_daily (
      ...
      PRIMARY KEY (summary_date, sector)
    );

    CREATE INDEX IF NOT EXISTS idx_sector_summary_daily_date
      ON sector_summary_daily(summary_date);

    CREATE INDEX IF NOT EXISTS idx_sector_summary_daily_sector
      ON sector_summary_daily(sector);
    """

    logger.info("Ensuring sector_summary_daily exists...")
    ...
    for sector, sector_df in df.groupby("sector"):
        rows.append(summarize_sector(sector_df.copy(), summary_date))

    if not rows:
        logger.warning("No sector rows were generated.")
        return
    ...
    ON CONFLICT (summary_date, sector) DO UPDATE SET
    ...
    logger.info(f"Inserting sector_summary_daily for {summary_date}...")
    with engine.begin() as conn:
        conn.execute(text(upsert), rows)

    logger.info("✅ sector_summary_daily updated successfully")
